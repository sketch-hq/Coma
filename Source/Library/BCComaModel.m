...
- (void)preprocessClasses
{
  ...
  // add the classInfo as a direct property - this allows templates to access this information explicitly, even in nested contexts where it might be masked
  [classes enumerateKeysAndObjectsUsingBlock:^(id className, NSMutableDictionary* info, BOOL *stop) {
    info[@"classInfo"] = [info copy];
  }];
...
- (void)addFiltersToDictionary:(NSMutableDictionary*)dictionary
{
  dictionary[@"initialCapital"] = [GRMustacheFilter filterWithBlock:^id(id value) {
    ...
    return [string stringWithInitialCapital];
  }];
